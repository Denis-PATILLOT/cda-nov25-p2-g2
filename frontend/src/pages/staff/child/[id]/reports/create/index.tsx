...
import {
    NewReportInput,
    useChildByIdQuery,
    useChildWithGroupAndPlanningsQuery,
...
const CreateReportPage = () => {
    ...
    const { refetch } = useChildWithGroupAndPlanningsQuery({
        variables: { childId: Number(id) },
    });

    const { data: dataChild, error } = useChildByIdQuery({
    ...
    const handleSubmitForm = async (data: NewReportInput) => {
        try {
            ...
            if (createdReport) {
                await refetch();
                router.push(
                    `/staff/child/${id}/reports/${createdReport.data?.createReport.id}?created=true`,
                    `/staff/child/${id}/reports/${createdReport.data?.createReport.id}`,
                );
            }
        } catch (err) {
            ...
            <div className="max-w-full mx-auto md:max-w-[1000px]">
                {errorSubmit && errorReport && (
                    <p className="text-red-500 px-5 mx-5 my-2 alert bg-red-100 border relative border-red-500 md:text-xl md:mx-52">
                        ...
                        {errorReport.message.match(
                            /[0-9]{2}\/[0-9]{2}\/[0-9]{4}/,
                        ) && (
                                <span>
                                    Planning inexistant pour cette date
                                    <br /> Pour créer un planning à cette date,
                                    cliquer sur ce{" "}
                                    <Link
                                        href={`http://localhost:3000/staff/planning/create?date=${errorReport.message.match(/[0-9]{2}\/[0-9]{2}\/[0-9]{4}/)![0]}`}
                                        as={
                                            "http://localhost:3000/staff/planning/create"
                                        }
                                        className="text-[#1b3c79] hover:underline"
                                    >
                                        lien
                                    </Link>
                                </span>
                            )}
                        {(errorReport as unknown as any).errors[0].extensions
                            .code === "REPORT ALREADY EXISTED" &&
                            "Compte-rendu déjà créé pour l'enfant à cette date !"}
                ...
                <div className="w-full flex justify-start items-center mt-5 mb-8 text-[#1b3c79]">
                    ...
                    <img
                        src={child?.picture}
                        alt={`picture of ${child?.firstName} ${child?.lastName}`}
                    ...
                    {child && (
                        <>
                            ...
                            <h1>
                                ...
                                />{" "}
                                Choix date compte-rendu
                                <input
                                    {...register("date", {
                                        // register : nécessaire pour inscription du champ dans l'objet data soumis
                            ...
                            <div className="w-[90%] mx-auto">
                                ...
                                <form onSubmit={handleSubmit(handleSubmitForm)}>
                                    ...
                                    <textarea
                                        ...
                                        name="staff_comment"
                                        disabled={!isPresent}
                                        placeholder={`saisir le commentaire du jour pour ${child.firstName} ${child.lastName}...`}
                                        rows={4}
                                    ...
                                    <input
                                        ...
                                        type="text"
                                        name="picture"
                                        placeholder="url photo à indiquer"
                                    ...
                                    />
                                    <p className="text-red-500 mb-1 text-xs">
                                        {errors.picture?.type === "pattern" &&
                                            "choisir une url valide"}
                                    </p>
                                    ...
                                    <div className="flex justify-between md:justify-center md:gap-4">
                                        ...
                                        />
                                        <label
                                            htmlFor="baby_mood_bad"
                                            className={`${!isPresent ? "text-gray-400" : ""}`}
                                        >
                                            difficile
                                        </label>
                                        <p className="text-red-500 mb-1 text-xs">
                                            {errors.baby_mood?.type ===
                                                "required" &&
                                                "humeur du jour requise"}
                                        </p>
                                    </div>
                                    <button
                                        type="submit"
