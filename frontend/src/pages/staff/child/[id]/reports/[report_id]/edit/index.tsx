...
import { useReportByIdQuery, useUpdateReportMutation } from "@/graphql/generated/schema";
...
const EditReport = () => {
    ...
    const { data, error, refetch } = useReportByIdQuery({variables: {reportId: Number(report_id)}})
                ...
                <div className="w-[90%] px-4 py-1 bg-[#FEF9F6] rounded-2xl text-[#1b3c79] font-semibold mx-auto border-3 border-[#FFD771]">
                    ...
                    <>
                        ...
                        <div className="w-[90%] mx-auto">
                            <p className="text-xs underline text-center my-3"> modification Compte-rendu</p>
                            <form onSubmit={handleSubmitForm}>
                                <label htmlFor="isPresent" className="mr-3 mb-3">Présent</label><input type="checkbox" id="isPresent" name="isPresent" defaultChecked={report.isPresent} onChange={() => setIsPresent(!isPresent)}  />
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <label htmlFor="baby_mood_good">bonne</label>
                                    <input type="radio" name="baby_mood" id="baby_mood_average" value="neutral" disabled={!isPresent} defaultChecked={report.baby_mood === "neutral"} />
                                    <label htmlFor="baby_mood_average">moyen</label>
                                    <input type="radio" name="baby_mood" id="baby_mood_bad" value="bad" disabled={!isPresent} defaultChecked={report.baby_mood === "bad"} />
                                    <label htmlFor="baby_mood_bad">difficile</label>
                                </div>
                                <button type="submit" className="text-center mx-auto block border-2 border-amber-300 bg-[#FEE8B6] px-5 py-2 rounded-2xl my-3 text-xl cursor-pointer hover:bg-[#FFF999]">{loading ? "Modification..." : "Valider"}</button>
                            </form>
                        </div>
                    </>
                    }
