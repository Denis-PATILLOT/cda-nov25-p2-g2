...
const ChildReports = () => {
    ...
    if (user && isAuthenticated && isStaff && group) {
        ...
        const years = new Set(
            childReports?.map((report) => new Date(report.date).getFullYear()),
        );

        const childReportsFiltered = childReports
            ?.filter(
                (report) => new Date(report.date).getMonth() === reportMonth,
            )
            .filter(
                (report) => new Date(report.date).getFullYear() === reportYear,
            );
        ...
        return (
            <Layout pageTitle="Staff - comptes-rendus">
                {error && (
                    <p className="text-red-500 alert border-red-500 text-center m-2">
                        ...
                        {error instanceof CombinedGraphQLErrors &&
                            ...
                            "Vous ne pouvez accéder à cet enfant"}
                        {error instanceof CombinedGraphQLErrors &&
                    ...
                    <div className="max-w-full mx-auto md:max-w-[1200px] ">
                        <div className="w-[full] flex justify-start items-center mt-5 mb-8 text-[#1b3c79] md:w-[full] md:mx-auto">
                            ...
                            <img
                                ...
                                className="h-[130px] w-[130px] ml-5 object-cover rounded-[50%] border-3 border-[#ffdd23] absolute md:relative md:left-35 md:h-[200px] md:w-[200px] md:ml-0"
                                loading="lazy"
                            />
                            <p className="w-[67%] h-[100px] rounded-4xl bg-[#FEF9F6] border-3 border-[#FFD771] ml-25 text-center pt-3 pl-5 flex justify-end md:w-[65%]">
                        ...
                        <div className="w-[90%] p-4 bg-[#FEF9F6] rounded-2xl text-[#1b3c79] font-semibold text-center border-3 border-[#FFD771] mx-auto min-h-[450px]">
                            ...
                            <Link href={`/staff/child/${id}/reports/create`}>
                                <button className="bg-[#fcde36] p-2 rounded-xl text-xs text-gray-50 hover:bg-[#ffbb25] hover:text-white cursor-pointer mb-2 text-right">
                                    Créer compte-rendu
                                </button>
                            </Link>
                            ...
                            {childReports && childReports.length === 0 && (
                                <p className="md:text-center">
                                    Aucun compte-rendu pour cet enfant
                                </p>
                            )}
                            ...
                            {child &&
                                ...
                                childReports.length > 0 && (
                                    <>
                                        <div className="flex gap-5 justify-evenly lg:justify-center">
                                            <div className="my-3">
                                                <select
                                                    name="month"
                                                    id="month"
                                                    className="border-2 border-[#FFD771] py-1 px-2"
                                                    value={reportMonth}
                                                    onChange={
                                        ...
                                        <div className="flex w-full mt-3 flex-wrap justify-start gap-3 md:justify-center">
                                            {childReports &&
                                                childReportsFiltered &&
                                                childReportsFiltered.length > 0 ? (
                                                childReportsFiltered.map(
                                                    (report) => (
                                                        <div
                                                            key={report.id}
                                                            className="w-[30%] px-4 bg-[#FEF9F6]  text-xs rounded-2xl border-2 border-[#FFD771] hover:bg-[#FEE8B6] lg:w-[20%]"
                                                        >
                                                            <Link
                                                                href={`/staff/child/${child.id}/reports/${report.id}`}
