...
export default function StaffPlanning() {
    ...
    const { data, loading, refetch } = useGetAllPlanningsByGroupQuery({
    ...
    const [planningYear, setPlanningYear] = useState<number>(
    ...
    if (user && isAuthenticated && isStaff && group) {
        ...
        const years = new Set(
            groupPlannings?.map((planning) =>
                new Date(planning.date).getFullYear(),
            ),
        );

        const groupPlanningsFiltered = groupPlannings
            ?.filter(
                (planning) =>
                    new Date(planning.date).getMonth() === planningMonth,
            )
            .filter(
                (planning) =>
                    new Date(planning.date).getFullYear() === planningYear,
            );
                    ...
                    <div className="w-[90%] p-4 bg-[#FEF9F6] rounded-2xl text-[#1b3c79] font-semibold text-center border-3 min-h-[600px] border-[#FFD771] mx-auto">
                        <h1 className="m-2 mb-2">
                            <img
                                ...
                                loading="lazy"
                            />{" "}
                            Plannings - {group.name}
                        </h1>
                        <Link href={"/staff/planning/create"}>
                            <button className="bg-[#ffdd23] p-2 rounded-xl text-xs text-gray-50 hover:bg-[#ffbb25] cursor-pointer mb-2 text-right">
                                Créer planning
                            </button>
                        </Link>
                        ...
                        <div className="flex w-full flex-wrap justify-start gap-3 mt-5 md:justify-center">
                            {groupPlanningsFiltered &&
                                groupPlanningsFiltered.length > 0 ? (
                                groupPlanningsFiltered.map((planning) => (
                                    <div
                                        key={planning.id}
                                        className="w-[30%] px-4 bg-[#FEF9F6]  text-xs rounded-2xl border-2 border-[#FFD771] hover:bg-[#FEE8B6]"
                                    >
                                        <Link
                                            href={`/staff/planning/${planning.id}`}
