    ...
    <Layout pageTitle="Rapport des enfants - Admin">
      <div className="mx-auto w-full max-w-[420px] px-4 pt-2 pb-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-[16px] font-semibold">Rapport des enfants</h1>
          <div className="w-10" />
        </div>

        {/* Barre de recherche */}
        <div className="mt-2 flex items-center h-9 rounded-lg bg-white/80 border-2 border-(--color-primary) px-3 shadow-sm transition-all duration-200 hover:shadow-md focus-within:shadow-md focus-within:scale-[1.01]">
        ...
        <div className="mt-2 relative w-fit">
          <button
            ...
            className="flex items-center h-9 rounded-xl border-2 border-(--color-primary) bg-white/80 px-3 shadow-sm text-[12px] gap-2"
          >
            <span className="text-gray-600">
              {selectedDate === today ? "Aujourd'hui" : new Date(selectedDate).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}
            </span>
            <span className={`text-gray-400 transition-transform duration-200 ${dateDropdownOpen ? "rotate-180" : ""}`}>▾</span>
          </button>

          {dateDropdownOpen && (
            <div className="absolute left-0 top-10 z-10 rounded-xl border-2 border-(--color-primary) bg-white overflow-hidden shadow-lg min-w-[190px]">
              {dates.length === 0 && (
                <div className="px-3 py-2 text-[12px] text-gray-400">Aucune date disponible</div>
              )}
              {dates.map((d) => (
                <button
                  key={d}
                  ...
                  className={`w-full text-left px-3 py-2 text-[12px] border-b border-gray-50 last:border-0 hover:bg-orange-50 ${selectedDate === d ? "font-semibold" : ""}`}
                >
                  {d === today ? "Aujourd'hui" : new Date(d).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}
                </button>
              ))}
            </div>
          )}
        ...
        <div className="mt-4 flex items-center gap-2">
          <img src="/boutons/calendrier.png" alt="calendrier" className="h-8 w-8 object-contain" />
          <span className="text-[14px] font-semibold">
            {selectedDate === today
              ? "Date du jour"
              : (() => {
                  const s = new Date(selectedDate).toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long" });
        ...
        {!loading && !error && (
          <div className="mt-3 flex flex-col gap-3">
            ...
            {filteredReports.map((r) => (
              <div
                key={r.id}
                className="rounded-2xl bg-white/80 border-2 border-(--color-secondary) px-3 py-3 shadow-md"
              >
                ...
                <button
                  ...
                  className="mt-3 w-full border-t border-(--color-primary) pt-2 text-[13px] font-medium text-center hover:opacity-70 transition-opacity"
                >
                  Voir rapport
                </button>
              </div>
