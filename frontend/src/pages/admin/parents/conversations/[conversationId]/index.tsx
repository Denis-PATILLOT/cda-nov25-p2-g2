...
const Conversation = () => {
  ...
  const handleSubmitForm = async (event: React.SyntheticEvent<HTMLFormElement>) => {
            ...
            <div key={parent?.id} className="relative rounded-2xl bg-white/80 border-2 border-(--color-secondary) px-3 py-3 shadow-md md:px-6 md:py-5 md:rounded-3xl">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 md:gap-5">
                  <img src={parent.avatar ?? "/admin/parentavatar.webp"} alt={`${parent.first_name} ${parent.last_name}`} className="h-12 w-12 rounded-full object-cover border-2 border-(--color-primary) shrink-0 md:h-20 md:w-20" />
