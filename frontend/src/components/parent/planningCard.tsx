...
export default function PlanningCard({ apiPlanning }: Props) {
    const formattedDate = new Date(apiPlanning.date).toLocaleDateString(
        "fr-FR",
        {
            weekday: "long",
            day: "numeric",
    ...
    const planning: PlanningSection[] = [
        {
            ...
            items: [
                {
                    icon: "/boutons/puzzle_activite.webp",
                    title: "Activité",
                    description:
                        apiPlanning.morning_activities ?? "Non renseigné",
                },
                {
                    icon: "/boutons/dormir.webp",
                    title: "Sieste",
            ...
            items: [
                {
                    ...
                    title: "Activité",
                    description:
                        apiPlanning.afternoon_activities ?? "Non renseigné",
                },
                {
                    icon: "/boutons/dormir.webp",
                    title: "Sieste",
...
function SectionTitle({ children }: { children: React.ReactNode }) {
    return (
        <div className="mt-5 flex items-center gap-3">
            <img
                src="/boutons/etoile.webp"
                alt=""
                className="h-7 w-7 shrink-0 object-contain"
            />
            <h3 className="text-[18px] font-extrabold tracking-tight text-blue-900">
                {children}
            </h3>
        </div>
    );
        ...
        <div className="mt-3 rounded-[22px] border-[4px] border-yellow-300 bg-[#fdf1c7]/75 px-3 py-3">
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1 leading-tight">
                    <p className="text-[18px] font-extrabold text-blue-900">
                        {title}
                    </p>
